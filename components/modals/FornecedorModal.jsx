...
export default function FornecedorModal() {
    ...
    return (
        <div {...modal.props} className="fixed inset-0 z-[80] flex items-stretch sm:items-center justify-center p-0 sm:p-4 bg-slate-900/40 dark:bg-black/80 glass no-print transition-all cursor-pointer animate-modal-backdrop">
            <div onClick={e => e.stopPropagation()} className="bg-fundo rounded-none sm:rounded shadow-2xl w-full max-w-none sm:max-w-md h-full sm:h-auto overflow-hidden cursor-default border border-borda-fraca animate-modal-in">
                ...
                <div className="p-6 space-y-4">
                    <div>
                        <label className="block text-corpo font-medium mb-1.5 text-tinta-corpo">Nome / Local *</label>
                        <input type="text" required value={novoFornecedor.nome} onChange={e => setNovoFornecedor({...novoFornecedor, nome: e.target.value})} className="w-full bg-elevado border border-borda rounded px-3 py-2 text-compacto outline-none focus:border-brand transition dark:text-[#EDEDED] font-medium" placeholder="Ex: Gráfica XYZ, Futura..." />
                    </div>
                    <div>
                        <label className="block text-corpo font-medium mb-1.5 text-tinta-corpo">Tipo de Fornecedor</label>
                        <CustomSelect
                            value={novoFornecedor.tipo || 'Terceirização'}
                            onChange={(val) => setNovoFornecedor({...novoFornecedor, tipo: val})}
                            className="w-full bg-elevado border border-borda rounded px-3 py-2 text-compacto outline-none focus:border-brand transition dark:text-[#EDEDED] font-medium cursor-pointer"
                            options={[
                    ...
                    <div>
                        <label className="block text-corpo font-medium mb-1.5 text-tinta-corpo">Contato (Telefone, E-mail)</label>
                        <input type="text" value={novoFornecedor.contato} onChange={e => setNovoFornecedor({...novoFornecedor, contato: e.target.value})} className="w-full bg-elevado border border-borda rounded px-3 py-2 text-compacto outline-none focus:border-brand transition dark:text-[#EDEDED] font-medium" placeholder="Ex: (11) 9999-9999" />
                    </div>
                    <div>
                        <label className="block text-corpo font-medium mb-1.5 text-tinta-corpo">Observações</label>
                        <textarea rows="3" value={novoFornecedor.observacoes} onChange={e => setNovoFornecedor({...novoFornecedor, observacoes: e.target.value})} className="w-full bg-elevado border border-borda rounded px-3 py-2 text-compacto outline-none focus:border-brand transition dark:text-[#EDEDED] font-medium resize-none custom-scrollbar" placeholder="Dados bancários, prazo padrão..." />
                    </div>
                </div>
                <div className="px-6 py-4 bg-gray-50 dark:bg-darkHover/30 border-t border-borda-fraca flex justify-end gap-3">
                    ...
                    }} className="bg-brand hover:bg-brandHover text-white px-5 py-2 text-compacto font-semibold rounded shadow-sm transition">Salvar</button>
                </div>
            </div>
        </div>
    );
