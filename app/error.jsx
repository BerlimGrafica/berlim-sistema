...
export default function Error({ error, unstable_retry }) {
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-fundo p-6 select-none">
            <div className="w-full max-w-md bg-superficie border border-borda rounded-xl p-8 shadow-sm flex flex-col items-center gap-4 text-center">
                ...
                <div>
                    <h2 className="text-lg font-black text-tinta tracking-tight">O sistema encontrou um erro</h2>
                ...
                {(error?.message || error?.digest) && (
                    <p className="w-full text-mini font-mono text-tinta-fraca bg-sutil border border-borda-fraca rounded px-3 py-2 break-words">
                        {error?.message}{error?.digest ? ` (ref: ${error.digest})` : ''}
                    </p>
                )}
                <div className="flex gap-3 mt-1">
                    <button type="button" onClick={() => window.location.reload()} className="bg-brand hover:bg-brandHover text-white px-4 py-2 text-corpo rounded-md font-semibold shadow-sm transition">
                        Recarregar o sistema
                    </button>
                    <button type="button" onClick={() => unstable_retry()} className="px-4 py-2 text-corpo rounded-md font-semibold border border-borda text-gray-600 dark:text-[#EDEDED] hover:bg-sutil transition">
                        Tentar de novo
                    </button>
                </div>
            </div>
