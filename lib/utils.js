        ...
        <div className="relative" ref={containerRef}>
            ...
            {isOpen && typeof document !== 'undefined' && createPortal(
                <>
                    ...
                    <div style={popoverStyle} className="z-[60] bg-white dark:bg-darkCard border border-gray-200 dark:border-darkBorder rounded-lg shadow-2xl p-4 w-72">
...
export function CalculadoraBanner() {
    ...
    return (
        <div className="bg-white dark:bg-darkCard p-6 rounded border border-gray-200 dark:border-darkBorder border-t-[3px] border-t-brand dark:border-t-brand">
            <h1 className="text-2xl lg:text-3xl font-black text-gray-900 dark:text-white tracking-tight mb-6">Calculadora de Banner / Lona</h1>
...
export function CalculadoraAdesivo({ produtos }) {
    ...
    return (
        <div className="bg-white dark:bg-darkCard p-6 rounded border border-gray-200 dark:border-darkBorder border-t-[3px] border-t-brand dark:border-t-brand">
            <h1 className="text-2xl lg:text-3xl font-black text-gray-900 dark:text-white tracking-tight mb-6">Calculadora de Adesivos (Vinil)</h1>
...
export function CalculadoraCasamento() {
    ...
    return (
        <div className="bg-white dark:bg-darkCard p-6 rounded border border-gray-200 dark:border-darkBorder border-t-[3px] border-t-brand dark:border-t-brand">
            <h1 className="text-2xl lg:text-3xl font-black text-gray-900 dark:text-white tracking-tight mb-6">Calculadora de Papelaria de Casamento</h1>
            ...
            <div className="space-y-6 fade-in">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <select value={manual.papel} onChange={e => setManual({...manual, papel: e.target.value})} className="w-full bg-white dark:bg-darkElevated border border-gray-200 dark:border-darkBorder rounded px-3 py-2 text-[13px] outline-none focus:border-brand dark:text-white">
                                {Object.keys(precosPapel).map(p => <option key={p} value={p}>{p}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="block text-[11px] font-semibold text-gray-500 mb-1">Só Furo?</label>
                            <select value={manual.furo} onChange={e => setManual({...manual, furo: e.target.value})} className="w-full bg-white dark:bg-darkElevated border border-gray-200 dark:border-darkBorder rounded px-3 py-2 text-[13px] outline-none focus:border-brand dark:text-white">
                                <option value="Sem furo">Sem furo</option>
                                <option value="Somente furo">Somente furo</option>
                                <option value="Furo e Ilhós">Furo e Ilhós</option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div className="bg-brand/10 p-4 rounded-lg border border-brand/20 flex flex-col gap-3">
                        <div className="flex items-center justify-between">
                            <span className="font-semibold text-brand">Valor Final</span>
                            <span className="text-2xl font-black text-brand">R$ {resultManual.valorFinal.toFixed(2).replace('.', ',')}</span>
                        </div>
                        <div className="pt-3 border-t border-brand/20 flex gap-2">
                            <input type="text" readOnly value={resultManual.textoWhatsapp} className="flex-1 bg-white/50 dark:bg-darkBg/50 text-[11px] p-2 rounded border border-brand/20 outline-none text-gray-700 dark:text-gray-300 font-mono" />
                            <button onClick={() => {
                                navigator.clipboard.writeText(resultManual.textoWhatsapp);
                                alert("Texto copiado!");
                            }} className="bg-brand text-white px-3 py-1 rounded text-[11px] font-bold hover:bg-brandHover transition shrink-0 flex items-center gap-1">
