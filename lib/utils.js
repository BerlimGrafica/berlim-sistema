...
export function MultiSelectDropdown({ value, options, onChange, className, disabled, placeholder = "Indefinido" }) {
    ...
    return (
        <div className="relative w-full" ref={containerRef}>
            ...
            >
                <div className="flex items-center gap-1.5 truncate">
                    <span className={`truncate font-medium ${selectedArr.length > 0 ? 'text-brand dark:text-brand' : 'text-gray-500 dark:text-gray-400'}`} title={selectedArr.length > 0 ? selectedArr.join(', ') : ''}>
