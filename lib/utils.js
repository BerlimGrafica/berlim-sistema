    ...
    return (
        <div className="relative" ref={containerRef}>
            <div
                ...
                className={`flex justify-between items-center cursor-pointer select-none ${className} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
                <span className={`truncate ${(startValue || endValue) ? "text-gray-900 dark:text-[#EDEDED]" : "text-gray-400 dark:text-gray-600"}`}>
                    {label}
                </span>
                <div className="flex items-center gap-1 shrink-0 ml-1">
                    {(startValue || endValue) && (
                        <button type="button" onClick={limparPeriodo} aria-label="Limpar período" className="p-0.5 text-gray-400 hover:text-brand transition">
