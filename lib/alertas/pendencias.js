...
const REGRAS = [
    ...
    ({ usuario, pedidos, hoje }) => {
        ...
        return pedidos
            ...
            .map(p => ({
                // Prazo de amanhã é azul, não âmbar: ainda há um dia inteiro para
                // buscar, então é trabalho a programar e não urgência. O âmbar
                // fica reservado ao que precisa de ação hoje — e é o vermelho que
                // assume quando o prazo chega ou passa.
                gravidade: p.prazo <= hoje ? 'critico' : 'tarefa',
                id: `futura:${p.id}`,
